<x-layout>
    <div class="container min-vh-100 d-flex align-items-center justify-content-center">
        <form action="{{route('admin.news.create')}}" method="POST" class="card p-5" enctype="multipart/form-data">
            ...
            @enderror
            <input class="form-control mb-3" type="text" placeholder="Автор" name="author" value="{{old('author')}}" required>
            @error('author')
                <p class="text-danger mb-3">{{$message}}</p>
            @enderror
            ...
            <p class="text-danger mb-3">{{$message}}</p>
            @enderror
            <input class="form-control mb-3" type="text" placeholder="Информация" name="body" value="{{old('body')}}" required>
            @error('body')
                <p class="text-danger mb-3">{{$message}}</p>
            @enderror
